    ...
    device
      .update(|packet| match packet {
        ...
        PacketRef::NavPvt(sol) => {
          ...
          let has_posvel =
            sol.fix_type() == GpsFix::Fix3D || sol.fix_type() == GpsFix::GPSPlusDeadReckoning;
        ...
        PacketRef::Unknown(unknown) => {
          println!(
            "\nUnknown UBX packet - Class: {}, Msg ID: {} \nRAW: {:?}",
            unknown.class, unknown.msg_id, unknown
          );
          let i = |idx| u32::from_le_bytes(unknown.payload[idx..idx + 4].try_into().unwrap());
          let s = |idx| i(idx) as i32;

          println!(
            ">>>> Payload: {:?}",
            UbxNavPvt {
              i_tow: i(0),
              year: u16::from_le_bytes(unknown.payload[4..6].try_into().unwrap()),
              month: unknown.payload[6],
              day: unknown.payload[7],
              hour: unknown.payload[8],
              min: unknown.payload[9],
              sec: unknown.payload[10],
              valid: unknown.payload[11],
              fix_type: unknown.payload[20],
              lat: s(28) as f64 / 1e7,
              lon: s(24) as f64 / 1e7,
              height: s(32),
              h_msl: s(36),
